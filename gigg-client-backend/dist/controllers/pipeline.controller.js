...
exports.getPublicPipeline = getPublicPipeline;
...
function clockWindowFor(task, job, completion) {
    ...
    if (completion?.manually_reopened_at) {
        ...
        return { opensAt: reopenedAt, deadline: new Date(reopenedAt.getTime() + openAfter * 60000) };
    }
    ...
    const openAfter = task.open_minutes_after !== undefined ? task.open_minutes_after : 30;
    const opensAt = new Date(anchor.getTime() - openBefore * 60000);
    const deadline = new Date(anchor.getTime() + openAfter * 60000);
...
async function applyTimeBasedTransitions(completions, tasksById, job) {
    const now = Date.now();
    const toFail = [];
    const toAutoApprove = [];
    for (const c of completions) {
        if (c.status !== 'in_progress' && c.status !== 'submitted' && c.status !== 'not_started')
            continue;
...
async function authorizeCompletionAccess(req, applicationId) {
    if (req.user?.role === 'admin')
        return { ok: true, isWorker: true, isEmployer: true };
...
async function getPublicPipeline(req, res) {
    const { shareToken } = req.params;
    let isSingleWorker = false;
    let singleWorkerApp = null;
    // 1. Check if shareToken matches an application ID directly
    const { data: appById } = await supabase_1.supabase
        .from('applications')
    ...
    if (appById) {
        ...
        try {
            const { data: appByToken } = await supabase_1.supabase
                .from('applications')
                .select('id, job_id, worker_id, status')
    ...
    const { data: jobById } = await supabase_1.supabase
        .from('jobs')
    ...
    if (jobById) {
        ...
        try {
            const { data: jobByToken } = await supabase_1.supabase
                .from('jobs')
                .select('id, title, location')
                .eq('pipeline_share_token', shareToken)
                .maybeSingle();
    ...
    if (isSingleWorker) {
        ...
        const { data: appsData, error: appsError } = await supabase_1.supabase
            .from('applications')
    ...
    const { data: kycDocs } = await supabase_1.supabase
        .from('kyc_documents')
        .select('user_id, full_name')
        .in('user_id', workerIds);
    const kycMap = new Map((kycDocs || []).map((k) => [k.user_id, k.full_name]));
    ...
    const { data: tasks } = await supabase_1.supabase
        ...
        .eq('job_id', job.id)
        .order('sort_order', { ascending: true });
    const { data: jobRow } = await supabase_1.supabase
        .from('jobs')
    ...
    const workers = await Promise.all(applications.map(async (app, idx) => {
        ...
        const mappedCompletions = await Promise.all((completions || []).map(async (c) => {
            const task = tasksById.get(c.job_task_id);
